...
function App() {
  ...
  const [showDoctorCardsPopup, setShowDoctorCardsPopup] = useState(false);

  // 医生角色卡（从doctor.json生成）
  ...
  return (
    <div style={{ padding: 24, fontFamily: 'sans-serif', position: 'relative', display: 'flex', flexDirection: 'row', alignItems: 'flex-start' }}>
      {/* Doctor List Button - show when game started */}
      {gameStarted && (
        <>
          <DoctorListButton onClick={() => setShowDoctorCardsPopup(true)} />
          <DoctorCardPopup
            isOpen={showDoctorCardsPopup}
            onClose={() => setShowDoctorCardsPopup(false)}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
                  {DOCTOR_CARDS.map(card => (
                    <div key={card.key} style={{
                      ...
                      onClick={() => handleSelectDoctorCard(card)}
                      onMouseEnter={(e) => {
                      ...
                      <div style={{ fontWeight: 'bold', fontSize: 16, marginBottom: 8 }}>{card.name}</div>
